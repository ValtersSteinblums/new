...
class DrawingView(context: Context, attrs: AttributeSet): View(context, attrs) {
    ...
    override fun onTouchEvent(event: MotionEvent?): Boolean {
        ...
        when (event?.action) {
            // this event will be fired when the user puts finger on the screen
            MotionEvent.ACTION_DOWN -> {
                drawPath.color = color
                drawPath.brushThickness = brushSize.toFloat()
                drawPath.reset() // resetting the path before we set initial point
                drawPath.moveTo(touchX!!, touchY!!)
            }
            // this event will be fired when the user starts to move his finger
            // this will be fired continually until user picks up his finger
